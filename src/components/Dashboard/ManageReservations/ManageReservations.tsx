...
import { Box, Paper, Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, IconButton, Collapse, Divider, ToggleButtonGroup, ToggleButton, Button, TextField, InputAdornment } from '@mui/material';
...
export default function ManageReservations() {
  ...
  const updateReservation = require('../../../hooks/reservationHooks').useUpdateReservation();
  ...
  const handleCheckOut = async (reservation: Reservation) => {
    try {
      ...
      const apiRes = apiReservations.find(r => r.reservationId === reservation.id);
      if (apiRes) {
        ...
        if (roomId) {
          ...
          const roomTypeIndex = roomTypes.findIndex((rt: any) => rt.id === roomId);
  ...
  const { data: apiReservations = [], isLoading, refetch } = useGetMyReservations();
  ...
  const handleSaveChanges = async (data: { checkIn: Date | null; checkOut: Date | null; guests: number }) => {
    if (!selectedReservation?.id) return;
    try {
      await updateReservation.mutateAsync({
        id: selectedReservation.id,
        checkIn: data.checkIn ? data.checkIn.toISOString() : selectedReservation.checkIn,
        checkOut: data.checkOut ? data.checkOut.toISOString() : selectedReservation.checkOut,
        guests: data.guests
      });
      refetch();
    } catch (e) {
                ...
                <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                  {(() => {
                    ...
                    if (reservation.status === ReservationStatus.Confirmed && 
                        checkInDate <= today && now < checkOutDate) {
                      return (
                        <>
                          <Button
                            size="small"
                            ...
                            onClick={async () => {
                              ...
                              if (apiRes) {
                                ...
                                if (roomId) {
                                  ...
                                  await updateProperty.mutateAsync({ id: propertyId, roomTypes: updatedRoomTypes });
                                }
                              }
                              refetch();
                            }}
                    ...
                    if (reservation.status === ReservationStatus.Pending || 
                        (reservation.status === ReservationStatus.Confirmed && now < checkInDate)) {
                      return (
                        <>
                          {/* Edit button */}
                          <IconButton 
                          ...
                          {/* Show Approve button only for Pending reservations */}
      ...
      <Paper elevation={3} sx={{ p: 2 }}>
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <IconButton aria-label="Refresh" onClick={() => refetch()} sx={{ ml: 1 }}>
