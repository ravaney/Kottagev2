    ...
    >
      ...
      <Box sx={{ position: 'relative' }}>
        <CardMedia
          component="img"
          height="180"
          image={property.images ? Object.values(property.images)[0] as string : "https://via.placeholder.com/300x180?text=Property"}
          alt={property.name}
        />
        {alerts.length > 0 && (
          <Box 
      ...
      {/* Property Details */}
      <CardContent className="card-content" sx={{ flexGrow: 1, pb: 2, transition: 'background-color 0.2s ease-in-out' }}>
        <Typography 
          gutterBottom 
          variant="h6" 
          component="div" 
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
          {property.roomTypes && property.roomTypes.length > 0 && (
            <Typography variant="caption" color="text.secondary">
              {property.roomTypes.reduce((total, room) => total + (room.quantityAvailable || 0), 0)} rooms
            </Typography>
          )}
          {property.maxGuests && (
            <Typography variant="caption" color="text.secondary">
        ...
        {property.roomTypes && property.roomTypes.length > 0 && (
          <Typography variant="h6" color={Colors.blue} fontWeight={600} sx={{ mt: 1 }}>
            ${Math.min(...property.roomTypes.map(rt => rt.pricePerNight || 0))}/night
          </Typography>
        )}
      ...
      <CardActions sx={{ pt: 0, justifyContent: 'space-between', px: 2, pb: 2 }}>
        ...
        <Button 
          ...
          startIcon={property.isListed ? <PauseCircleIcon /> : <CheckCircleIcon />}
          onClick={(e) => {
            e.stopPropagation();
...
export default function PropertyConfig({
  ...
  const getPropertyAlerts = useCallback((propertyName: string): LowAvailabilityAlert[] => {
  ...
  const handleCardClick = useCallback((property: Kottage) => {
    navigate(`/MyAccount/Dashboard/properties/manage/${property.id}`);
  }, [navigate]);

  // Handle property listing toggle
  const handleToggleListing = useCallback(async (property: Kottage) => {
